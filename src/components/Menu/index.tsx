...
const UikitMenu = lazy(() =>
  import('@gravis.finance/uikit/dist/esm/widgets/Menu').then(({ Menu }) => ({
    default: Menu
  }))
)

const Menu: React.FC<{ loginBlockVisible?: boolean }> = ({
  loginBlockVisible,
  ...props
}) => {
  ...
  const balance = useCurrencyBalance(
    account as string,
    BASE_CURRENCIES[chainId as ChainId]
  )
  ...
  const transactionsHistoryModalComponent = React.useMemo(
