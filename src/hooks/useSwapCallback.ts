...
import { JSBI, Percent, Router, SwapParameters, Trade, TradeType } from '@gravis.finance/sdk'
    ...
    return {
      ...
      callback: async function onSwap(): Promise<string> {
                  ...
                  .catch((callError) => {
                    ...
                    switch (callError.reason) {
                      ...
                      case 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT':
                        errorMessage = t('errorMessages.willNotSucceed')
                        break
                      default:
                        errorMessage = t('errorMessages.timeoutError')
                    }
        ...
        if (!successfulEstimation) {
          ...
          throw new Error(t('errorMessages.contactSupport'))
        }
        ...
        return contract[methodName](...args, {
          ...
          .catch((error: any) => {
            ...
            if (error?.code === 4001) {
              throw new Error(t('errorMessages.transactionRejected'))
            } else {
              // otherwise, the error was unexpected and we need to convey that
              console.error(`Swap failed`, error, methodName, args, value)
              throw new Error(`${t('errorMessages.swapFailed')}: ${error.message}`)
            }
          })
      },
      error: null,
    }
  }, [t, trade, library, account, chainId, recipient, recipientAddressOrName, swapCalls, addTransaction])
}
